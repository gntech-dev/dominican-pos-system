      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {reportPreview && (
          <div className="bg-white rounded-lg shadow-md p-6">
            ...
            {/* Validation Status */}
            {reportPreview.summary.validationStatus && (
              <div className="mb-6 p-4 rounded-lg bg-gray-50">
                <h4 className="font-semibold mb-3">Estado de Validación</h4>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {reportPreview.reportType === '606' ? (
                    <>
                      <div className="flex items-center">
                        <div className={`w-3 h-3 rounded-full mr-2 ${
                          reportPreview.summary.validationStatus.allSuppliersHaveRNC ? 'bg-green-500' : 'bg-red-500'
                        }`}></div>
                        <span className="text-sm">
                          {reportPreview.summary.validationStatus.allSuppliersHaveRNC ? 'Todos los proveedores tienen RNC válido' : 'Algunos proveedores sin RNC'}
                        </span>
                      </div>
                      <div className="flex items-center">
                        <div className={`w-3 h-3 rounded-full mr-2 ${
                          reportPreview.summary.validationStatus.taxCalculationsValid ? 'bg-green-500' : 'bg-red-500'
                        }`}></div>
                        <span className="text-sm">
                          {reportPreview.summary.validationStatus.taxCalculationsValid ? 'Cálculos de impuestos válidos' : 'Errores en cálculos de impuestos'}
                        </span>
                      </div>
                    </>
                  ) : (
                    <>
                      <div className="flex items-center">
                        <div className={`w-3 h-3 rounded-full mr-2 ${
                          reportPreview.summary.validationStatus.allSalesHaveNCF ? 'bg-green-500' : 'bg-red-500'
                        }`}></div>
                        <span className="text-sm">
                          {reportPreview.summary.validationStatus.allSalesHaveNCF ? 'Todas las ventas tienen NCF' : 'Algunas ventas sin NCF'}
                        </span>
                      </div>
                      <div className="flex items-center">
                        <div className={`w-3 h-3 rounded-full mr-2 ${
            ...
            {reportPreview.reportType === '607' && reportPreview.summary.ncfBreakdown && (
              <div className="mb-6 p-4 rounded-lg bg-blue-50">
                <h4 className="font-semibold mb-3">Desglose NCF</h4>
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                  {Object.entries(reportPreview.summary.ncfBreakdown).map(([ncfType, count]) => (
            ...
            {reportPreview.summary.customerTypes && (
              <div className="mb-6 p-4 rounded-lg bg-green-50">
                <h4 className="font-semibold mb-3">Tipos de Cliente</h4>
                <div className="grid grid-cols-3 gap-4">
                  <div className="text-center">
                    <div className="text-lg font-bold text-green-600">{reportPreview.summary.customerTypes.withRNC}</div>
                    <div className="text-sm text-gray-600">Con RNC</div>
                  </div>
                  <div className="text-center">
                    <div className="text-lg font-bold text-green-600">{reportPreview.summary.customerTypes.withCedula}</div>
                    <div className="text-sm text-gray-600">Con Cédula</div>
                  </div>
                  <div className="text-center">
                    <div className="text-lg font-bold text-green-600">{reportPreview.summary.customerTypes.walkIn}</div>
            ...
            {/* Supplier Count for 606 */}
            {reportPreview.summary.supplierCount && (
              <div className="mb-6 p-4 rounded-lg bg-indigo-50">
                <h4 className="font-semibold mb-3">Proveedores</h4>
                <div className="text-center">
                  <div className="text-2xl font-bold text-indigo-600">{reportPreview.summary.supplierCount}</div>
                  <div className="text-sm text-gray-600">Proveedores únicos</div>
                </div>
              </div>
            )}
